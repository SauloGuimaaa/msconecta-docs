...
├── moldes/                       # Templates SVG/PNG do layout de feed e story (feed.svg, story.svg, camadas alpha; colunista_feed.svg/colunista_story.svg + alpha desde 2026-09-04, ver seção 3.3b)
...
├── gerar_coluna.py                # ENTRADA: gerador de feed.png + story.png para posts de COLUNA (colunista do site, desde 2026-09-04 — ver seção 3.3b)
├── colunistas.json                # Cadastro fixo (nome/foto/bio) por colunista, usado por gerar_coluna.py
...
- **Passo a passo resumido** (detalhe completo em `CLAUDE.md`):
  1. Extrai a URL da imagem de capa (`scrape_noticia()`) em cascata de prioridade (implementado 2026-08-06): **(a)** procura o `<a href>` que envolve a `<img>` de capa no HTML e aponta direto para o arquivo original no CMS (`msconecta.com.br/images/noticias/...`), sem passar pelo proxy de imagens do site; **(b)** se não achar, extrai o parâmetro `src=`/`url=` de dentro de uma URL do proxy `load.websg.app.br` (em `og:image`, `twitter:image` ou `<img src>`), com URL-decode e tratamento de `&amp;`; **(c)** se nada bater (ex: notícia sem imagem de capa própria, vídeo embutido), cai no comportamento legado (`og:image`/seletores de `<img>`, forçando `w=2400&h=1600` quando a URL é do proxy WEBSG). Depois, baixa essa URL em até 3 estágios de fallback: download direto → proxy público (`codetabs`) → PC Windows via Tailscale (`http://100.108.170.120:8765/fetch-binary`).
     **Cache em memória por execução (desde 2026-09-17)**: `baixar_imagem(url)` guarda o resultado num dict de módulo (`_cache_imagem_baixada`), válido só durante o processo atual. Como STORY e FEED cada um chama `carregar_fundo()`→`baixar_imagem()` de forma independente dentro da mesma execução, isso evita baixar a mesma imagem duas vezes por geração "ambos" — relevante porque, com o PC Windows lento/fora do ar, cada download não cacheado pode levar até ~180s só no estágio de fallback do PC, e sem cache esse custo era pago duas vezes. Retorna `.copy()` do objeto PIL em cada chamada (nunca o mesmo objeto entre story e feed, que processam a imagem de formas diferentes — seam carving só no feed). Ver `HISTORICO_MUDANCAS.md` 2026-09-17.
...
### 3.3b Geração de design de posts de COLUNA (colunistas do site) — desde 2026-09-04

- **Objetivo**: mesmo tipo de material visual do template de notícia (feed 1080×1350, story 1080×1920), mas para colunas de opinião assinadas por colunistas cadastrados do site, com um painel de identificação do colunista (foto/nome/bio) que a notícia não tem.
- **Arquivo**: `gerar_coluna.py`, moldes em `moldes/colunista_feed.svg`/`moldes/colunista_story.svg` (+ `_alpha.png` gerados via `cairosvg`), cadastro fixo em `colunistas.json`.
- **Gatilho**: manual — Saulo cola o link de uma coluna (`msconecta.com.br/coluna/{id}/{slug}`) no bot Telegram. `orquestrador.py:main()` detecta o padrão de URL **por regex, antes da classificação de intenção por IA** (determinístico, não depende do classificador acertar) e roteia para `acao_gerar_design_coluna()`, que chama `gerar_coluna.py` em vez de `gerar_tudo.py`.
- **Reaproveitamento explícito de `gerar_tudo.py`** (via `import gerar_tudo as gt`, sem duplicar nada): scraping (`_fetch_html()`, `_extrair_capa_via_ancora()`/`_extrair_capa_via_proxy_param()` — o `og:image` de uma coluna já aponta pro proxy WEBSG com o arquivo original, mesma cascata de prioridade da notícia), quebra de linha (`wrap_titulo()`, DP balanceado), crop/Vision (`carregar_fundo()`/`baixar_imagem()`), legenda (`montar_legenda()`), convenção de pasta (`criar_pasta_saida()`), fontes/desenho de texto (`carregar_fonte()`, `desenhar_texto_com_spacing()`, `normalizar_texto_fonte()`), cores (`AZUL_MS`, `BRANCO`).
- **Extração específica de coluna** (não existe no template de notícia): parágrafos via `#texto_release_format p` (mesma div de conteúdo do CMS WEBSG, já um dos seletores tentados por `scrape_noticia()`); colunista via `.widget_colunista a[href*="/colunista/"]` — o `{slug}` do link `/colunista/{id}/{slug}` é a chave de busca em `colunistas.json` (nome/foto/bio fixos, cadastrados uma vez por colunista; erro claro e não-inventado se o slug não estiver cadastrado ou estiver incompleto).
- **Destaque azul (1-2 linhas) no título — decisão por frase semântica, não por índice de linha**:
  1. Quebra de linha determinística via `gt.wrap_titulo()`, calculada **separadamente** para FEED (56px/880px) e STORY (64px/880px) — os dois podem quebrar o mesmo título de forma diferente.
  2. `decidir_linhas_destaque()` manda as linhas já quebradas do FEED para `claude-haiku-4-5-20251001` (mesmo padrão de chamada de `monitor_videos.py:avaliar_ia()` — `urllib.request` cru, JSON extraído por regex, timeout 15s), que escolhe 1-2 linhas por número, com base em importância semântica.
  3. **Fallback determinístico** (linha do meio, índice `n // 2`) se `ANTHROPIC_API_KEY` estiver ausente, a chamada falhar/der timeout, ou a resposta vier fora do formato esperado — sempre logado em `logs/destaque_titulo_coluna.log` com o motivo exato, nunca silencioso.
  4. As linhas escolhidas no wrap do FEED viram um intervalo de **índices de palavra** dentro da sequência original de palavras do título (não índice de linha) — esse intervalo é comparado por sobreposição contra o wrap independente do STORY, destacando lá as linhas que contêm qualquer palavra do intervalo-alvo. Isso garante que Feed e Story destaquem a mesma ideia semântica mesmo quando o ponto de quebra de linha é diferente entre os dois formatos (testado com um título que de fato quebra em número de linhas diferente entre FEED e STORY — remapeamento correto nos dois).
- **Fontes Arboria — gap conhecido**: `fontes/Arboria/` só tem Bold/Book/Light/BookItalic. O template de coluna pede Arboria-Black (nome do colunista) e Arboria-Medium (label "COLUNISTA"), que **não existem no projeto** — `gerar_coluna.py:FONTES_COLUNA` já usa o mesmo padrão de fallback por lista de `gerar_tudo.py:FONTES` (Black→Bold, Medium→Bold), visualmente aceitável no teste real, mas a fidelidade ao arquivo de design original fica pendente de Saulo enviar os arquivos de fonte corretos.
- **Fluxo de aprovação**: reaproveita `estado.json`/`ler_legenda()`/`tg_send_with_buttons()`/`image_approval_utils.registrar_msg_aprovacao()` de `acao_gerar_design()`, com a mesma estrutura de saída (`output/YYYY-MM-DD/COLUNA_titulo-slug/{feed,story}.png`, `legenda.txt`, `meta.json`) — `publicar_instagram.py` não precisou de nenhuma mudança. **Teclado de aprovação reduzido**: só "Postar agora"/"Programar horário"/"Cancelar" — "Reposicionar"/"Editor visual"/"Melhorar com IA" ficam de fora de propósito, porque esses três chamam `corrigir_posicao.py`/`editor_visual.py`, que por sua vez chamam `gerar_tudo.py --manual` (template de NOTÍCIA) internamente; usá-los sobre uma pasta de coluna sobrescreveria silenciosamente o resultado com o template errado. Suporte a reposicionamento de coluna é trabalho futuro (precisaria de `--tipo=coluna` nesses dois scripts).
- **Serviços externos**: os mesmos de `gerar_tudo.py` (seção 3.3) — Anthropic (Vision + decisão de destaque), PC Windows via Tailscale, proxies HTTP públicos.
- **Escopo aceito, não implementado**: Story do colunista é uma adaptação direta do Feed (mesma lógica de painel, sem CTA de swipe-up próprio) — o link sticker do story continua sendo aplicado pela automação de publicação já existente (Playwright no PC Windows), independente do conteúdo do PNG gerado aqui.
...
- **Fluxo**: lê pasta de output (`feed.png`, `story.png`, `legenda.txt`, `meta.json`) → faz upload das duas imagens ao **Cloudinary** → **duas etapas sequenciais e tecnicamente independentes dentro de `publicar_feed_e_story()`, sem gate entre elas** (a etapa de Story roda incondicionalmente depois da etapa de Feed, mesmo que o Feed tenha falhado): **(1) Feed** — cria container de mídia na Graph API do Instagram usando a URL do Cloudinary, aguarda `FINISHED` (`aguardar_pronto()`), publica (`publicar_container()`); **(2) Story** — **não usa a Graph API nem a URL do Cloudinary do story** (`story_url` é enviado ao Cloudinary mas nunca referenciado depois — achado colateral, upload desperdiçado, não corrigido por não causar o bug investigado em 2026-09-21): publica via `bash postar_story_vps.sh`, que aciona automação de navegador (Playwright) no **PC Windows**, usando o **arquivo local** `story.png` (não uma URL), para poder aplicar o link sticker. Depois publica também em Threads (reusa a URL do Cloudinary do feed) e Facebook Page. Também expõe `postar_link_canal_instagram()`, usada para postar o **link da notícia no canal do Instagram** (recurso "canal" do Instagram, separado do feed/story) — via chamada HTTP `POST http://100.108.170.120:8765/postar-canal` (automação no PC Windows, timeout 70s), disparada por AMBOS os caminhos de publicação: o loop `--processar` do cron E a publicação manual `--pasta ... --agora` (chamada por `orquestrador.py:acao_publicar_feed()` quando o editor aprova "postar agora" no Telegram).
...
**Modo de falha conhecido — FEED falhando silenciosamente enquanto o STORY sai normalmente (identificado e corrigido em 2026-09-21):** confirmado por evidência real em `instagram.log` que a Graph API do Instagram, ao criar o container do FEED logo após o upload ao Cloudinary (às vezes < 2s depois), ocasionalmente retorna `"Only photo or video can be accepted as media type."` (ou, mais raro, `"Media ID is not available"` na etapa de publicar) — um erro que a própria API **não marca como `is_transient`**, então o retry embutido em `criar_container()`/`publicar_container()` desistia na 1ª tentativa. A causa real **não é a imagem** (confirmado: a mesma URL do Cloudinary, buscada minutos depois, sempre retornou PNG válido nas dimensões corretas) — é uma corrida de propagação entre o Cloudinary/Cloudflare terminar de disponibilizar o asset e a Graph API tentar buscá-lo quase instantaneamente. Prova direta: numa publicação real de 2026-09-21 (`PANTANAL_corumba-completa-248-anos...`), o FEED foi rejeitado às 13:00:08 com esse erro, e o **Threads publicou com sucesso usando a MESMA URL do Cloudinary** ~90s depois (13:01:38) — sem nenhuma mudança na URL nem na imagem. Como a etapa de STORY não depende da Graph API/Cloudinary (ver "Fluxo" acima), ela publicava normalmente mesmo quando o FEED falhava dessa forma — dando exatamente a aparência de "story sai, feed não sai" reportada por Saulo. **Levantamento em `instagram.log` (14 dias, 2026-09-07 a 2026-09-21): 43 de 183 execuções de publicação tiveram esse erro no FEED** — bem mais frequente do que os "2-3 casos" percebidos, porque na maioria das vezes Threads/Facebook mascaravam o problema no retorno geral da função. **Achado adicional — mascaramento no `agendamentos.json`**: `publicar_feed_e_story()` retorna `sucessos > 0` (soma de feed+story+canal+threads+facebook bem-sucedidos) como único critério de sucesso; `processar_agendamentos()` grava esse booleano único como `status: "publicado"` — ou seja, mesmo com o FEED nunca publicado, o agendamento aparecia como `"publicado"` sem nenhuma distinção por plataforma (confirmado em `agendamentos.json` para os 3 casos de FEED falho de hoje, todos com `status: "publicado"`). **Fix aplicado**: (1) `criar_container()`/`publicar_container()` agora tratam essas duas mensagens de erro especificamente como retryable mesmo sem `is_transient=true` (`_erro_e_propagacao_cdn()`), usando o mesmo backoff já existente (20s/40s/60s) — suficiente, já que a propagação real leva segundos, não minutos; (2) `publicar_feed_e_story()` agora rastreia `feed_ok`/`story_ok` explicitamente e, sempre que só um dos dois tiver sucesso, loga um aviso completo e dispara alerta via `openclaw`/WhatsApp (mesmo canal já usado pelo restante do script) nomeando qual etapa falhou — nunca mais silencioso, mesmo que o retry não resolva o caso. **Não corrigido nesta sessão (fora do escopo, risco maior)**: o `status` gravado em `agendamentos.json` continua sendo um booleano único (`publicado`/`erro`) sem granularidade por plataforma — mudar esse schema exigiria revisar quem mais lê esse campo (dashboard, etc.) e ficou fora do escopo de um fix de baixo risco; o alerta via WhatsApp cobre a lacuna de "nunca mais silencioso" sem mexer no schema. Testado isoladamente (mocks de `requests.post`/`subprocess.run`): confirmado retry após erro de propagação CDN com sucesso na 2ª tentativa, nenhum retry adicional para erros reais não-transitórios (comportamento antigo preservado), e alerta disparado corretamente quando feed falha e story sucede. Ver `HISTORICO_MUDANCAS.md` 2026-09-21.
...
### Padrão recorrente de falha silenciosa em operações assíncronas/bloqueantes (risco sistêmico, registrado em 2026-08-13, 5º caso em 2026-08-16, 6º caso em 2026-08-28, 7º caso em 2026-09-01)
Sete bugs distintos, todos no mesmo padrão:
...
- **2026-09-01 (7º caso)** — `monitor_noticias.py:_fetch_via_pc_servidor()` só logava algo quando a chamada ao PC (`POST http://100.108.170.120:8765/fetch-url`) lançava exceção (timeout, conexão recusada). Uma resposta HTTP recebida normalmente mas com status ≠ 200, ou com `ok:false`/HTML vazio no corpo, retornava `None` **sem imprimir nada** — no log do cron (`monitor.log`) isso aparecia só como "Tentando PC servidor..." seguido imediatamente de "Tentando proxy...", sem nenhuma pista do motivo real. Foi exatamente esse silêncio que mascarou o incidente de DNS quebrado no PC Windows descrito na nova subseção abaixo ("Segunda via de bypass do Cloudflare silenciosamente quebrada") — a única forma de descobrir a causa real foi chamar a função manualmente fora do cron. **Fix**: a função agora loga o status HTTP + corpo da resposta quando ≠ 200, e o corpo da resposta quando 200 mas sem HTML válido — sem mudar nenhum comportamento de fallback, só tornando o motivo visível em `monitor.log` da próxima vez. Ver `HISTORICO_MUDANCAS.md` 2026-09-01.

Os sete casos têm a mesma forma: uma operação que roda em paralelo ao fluxo principal (thread daemon) ou tem um limite de tempo (timeout de rede/subprocess) sem que o código trate explicitamente o caminho de "não terminou a tempo", "levantou exceção" ou "terminou normalmente mas com um resultado de falha" — o resultado é sempre o mesmo, falha invisível para o usuário, sem log e sem mensagem no Telegram, só perceptível por auditoria manual de logs (ou execução manual fora do cron) depois do fato. **Não é uma correção pontual, é um sinal de risco sistêmico**: outras threads daemon e outras chamadas com timeout no projeto (`reel_handler.py`, `gerar_tudo.py`, `monitor_noticias.py`, etc.) devem ser revisadas com essa lente antes de assumir que estão corretas, mesmo sem um bug relatado ainda. Recomendação para revisões futuras: toda `threading.Thread(daemon=True)` que faz uma operação com efeito observável pelo usuário deveria ser aguardada (`.join()`) ou, se realmente precisa ser fire-and-forget, ter seu próprio tratamento de exceção que sempre loga e avisa; toda chamada com `timeout=` (subprocess, requests) deveria estar dentro de um `try/except` que trata o caso de estouro como um evento de primeira classe (log + aviso), não como algo que "não deveria acontecer"; e toda função de fetch com múltiplos critérios de sucesso (status HTTP **e** corpo/campo específico) deveria logar qual critério falhou, não só exceções de rede.
...
**Bug relacionado, causa raiz confirmada em 2026-09-17 — `acao_gerar_design()`/`acao_gerar_design_coluna()` sobrescreviam `ultima_pasta`/`aguardando_aprovacao` sem checar aprovação pendente (agora avisa, ainda não resolve estruturalmente):** os botões "Postar agora"/"Programar horário" (`callback_data: "pub:agora"`/`"pub:programar"`) atuam sobre `estado['ultima_pasta']` global — são literais fixos, **sem `msg_id`**, ao contrário de `imgia:feed:{msg_id}`/`undo:feed:{msg_id}`, que resolvem a pasta certa por mensagem clicada. Quando o usuário gera um novo design manual (URL colada) enquanto um anterior ainda aguarda resposta, `ultima_pasta` é sobrescrita sem nenhum aviso — a aprovação anterior nunca mais é referenciada por nenhum botão, e a notícia fica órfã (arquivos ficam em disco, mas nunca é agendada/publicada). Causa raiz confirmada da notícia "XP Investimentos" perdida em 2026-09-16 (gerada 18:07, nunca agendada nem publicada — ver `HISTORICO_MUDANCAS.md` 2026-09-17). **Fix aplicado**: nova `_avisar_se_aprovacao_pendente()` em `orquestrador.py`, chamada antes da sobrescrita em ambas as funções — envia aviso explícito via Telegram nomeando a notícia que será abandonada. **Não resolve o bug estrutural** (continua existindo só UM slot de "pendente" por vez); uma correção completa exigiria aprovações concorrentes como fila própria, com cada botão amarrado ao `msg_id`/pasta específicos — fica como trabalho futuro.
...
- **2026-09-17** — PC Windows caiu de fato na camada de rede do Tailscale (`tailscale status` = `offline, last seen 1h ago`, `tailscale ping` sem resposta — diferente do 5º caso acima, onde a rede/Tailscale CLI ainda respondia e só o watchdog estava travado) durante uma geração em andamento no editor visual, contribuindo para estourar o timeout de 300s (ver `HISTORICO_MUDANCAS.md` 2026-09-17 para o rastro exato). Saulo religou o PC na mesma sessão; `tailscale status` voltou a `active/direct` e, após alguns minutos (listener ainda inicializando), o endpoint HTTP (`:8765/status`) confirmou recuperação total (`200 OK`, `{"ok":true,"uptime":290s}`) — resolvido sozinho, sem necessidade do procedimento de timeout do `tailscale_watchdog.ps1` do 5º caso.
...
### Segunda via de bypass do Cloudflare silenciosamente quebrada: DNS quebrado no PC Windows, coincidindo com bloqueio real do IP da VPS (achado e investigado em 2026-09-01)

**Contexto do pedido**: Saulo questionou uma contradição aparente antes de mexer no painel do Cloudflare — se o scraping da listagem de notícias (`monitor_noticias.py`) já passa pelo PC Windows (IP residencial) especificamente para contornar o bloqueio de IP de datacenter do Cloudflare, por que o bloqueio do IP da própria VPS (`72.60.151.58`) estaria impedindo a coleta? A hipótese proposta era que a chamada da listagem nunca tivesse passado pelo PC (só a busca de imagem teria essa proteção) e que a chamada direta da VPS fosse a causa raiz.

**Investigação (código + evidência ao vivo, não só leitura):**
1. **Confirmado por leitura de código que a hipótese estava incorreta** — `monitor_noticias.py:scrape_noticias()` já tenta o PC Windows (`_fetch_via_pc_servidor()`, `/fetch-url`) **primeiro**, com os 2 proxies públicos só como fallback. Essa não é uma mudança desta sessão: foi implementada em 2026-08-16 (ver `HISTORICO_MUDANCAS.md` desse dia, "fix do descompasso de timeout no scraping") — antes disso o PC era a *última* via tentada, hoje é a primeira. Ou seja, a listagem de notícias tem, e já tinha antes deste incidente, a mesma proteção de IP residencial que a busca de imagem.
2. **Confirmado ao vivo, com chamadas reais, que o bloqueio do Cloudflare no IP da VPS é genuíno e atual**: `requests.get('https://msconecta.com.br/noticias', ...)` direto da VPS retorna HTTP 403 com a página "Access denied | msconecta.com.br used Cloudflare to restrict access" — não é suposição, é o texto literal da resposta do Cloudflare.
3. **Causa raiz real, revisada — dois problemas simultâneos e independentes, não um só:**
   - **(a) Cloudflare bloqueando a VPS** — confirmado no item 2 acima. Este bloqueio, por si só, não deveria quebrar a coleta, porque a rota que o contorna (via PC) já é tentada primeiro.
   - **(b) A rota de contorno (PC Windows) está com DNS quebrado** — `POST http://100.108.170.120:8765/fetch-url` (o listener HTTP do PC, que está no ar e saudável — `/status` responde `{"ok":true,"uptime":...}` normalmente) retorna `HTTP 500 {"ok":false,"erro":"getaddrinfo ENOTFOUND msconecta.com.br"}`. Testado com outros domínios (`example.com`, `google.com`) para isolar o escopo: **o mesmo erro `ENOTFOUND` ocorre para qualquer domínio**, não só `msconecta.com.br` — ou seja, não é um bloqueio específico do site, é resolução de DNS quebrada no PC Windows de forma geral (configuração de rede/DNS do Windows ou do adaptador Tailscale, não um problema de código nem do Cloudflare). Tentativa de diagnóstico adicional via SSH (`publicar_via_pc.sh`, via de acesso alternativa já documentada na seção 5) **falhou** (`Connection reset by 100.108.170.120 port 22`) — não foi possível investigar mais fundo nem corrigir remotamente a partir da VPS/Claude Code nesta sessão; o listener HTTP (porta 8765) responde normalmente, mas o SSH (porta 22) não, então o alcance de diagnóstico remoto ficou limitado ao que o próprio `/fetch-url` reporta.
   - **É a combinação das duas que derruba a coleta por completo**: a via primária (PC) falha por DNS quebrado, caindo no fallback de proxies públicos, que por sua vez também falham (522/520/500 — o mesmo bloqueio de IP de datacenter do Cloudflare que já afeta os proxies, documentado desde 2026-06-10 no cabeçalho de `monitor_noticias.py`) — e a chamada direta da VPS (que nem chega a ser tentada por `monitor_noticias.py`, mas foi testada aqui para confirmar o item 2) também é bloqueada. As três vias falhando ao mesmo tempo, por três motivos diferentes, é o que produz "Erro no scrape: Todos os proxies falharam apos retries." em `monitor.log` desde pelo menos 10:30 UTC de 2026-09-01 (6+ ciclos consecutivos de cron confirmados no log).
4. **Nenhuma mudança de reroteamento foi aplicada** (o pedido original de "reroteá-la para passar pelo PC" já está satisfeito desde 2026-08-16) — em vez disso, foi corrigida a falha silenciosa que impedia essa causa raiz de aparecer no log do cron (ver 7º caso do padrão de falha silenciosa acima), e o resto do diagnóstico foi devolvido a Saulo, já que a correção real (DNS do PC Windows) está fora do alcance de execução remota desta sessão (mesma classe de limitação já registrada nos casos de `tailscale_watchdog.ps1`/PC Windows anteriores — mudanças de rede na máquina precisam ser feitas fisicamente ou por quem tem acesso a ela).

**Pendência (aguardando Saulo)**: verificar a configuração de DNS do PC Windows (servidores DNS do adaptador de rede/Tailscale — `ipconfig /all`, testar `nslookup` de qualquer domínio no próprio PC) e corrigir; SSH (`publicar_via_pc.sh`) também está inacessível no momento (`Connection reset`), vale confirmar se é o mesmo problema de rede ou algo à parte (ex: serviço SSH do Windows parado). **Sobre o Cloudflare**: o bloqueio do IP da VPS existe e é real, mas não precisa ser resolvido no painel — assim que o DNS do PC for corrigido, a via primária (PC, IP residencial) volta a funcionar e a coleta se recupera sem depender do Cloudflare, exatamente como a arquitetura já prevê desde 2026-08-16.
...
### Fontes Arboria incompletas (achado em 2026-09-04, ao implementar o template de coluna)
`fontes/Arboria/` só contém Bold/Book/Light/BookItalic. O template de posts de coluna (`gerar_coluna.py`, ver seção 3.3b) precisa de Arboria-Black (nome do colunista) e Arboria-Medium (label "COLUNISTA"), que nunca foram adicionados ao projeto. Fallback por lista já em produção (Black→Bold, Medium→Bold, mesmo padrão de `gerar_tudo.py:FONTES`) — visualmente aceitável, mas não é o peso de fonte pretendido pela especificação original de design. Pendência: Saulo enviar os arquivos `.ttf` corretos (Arboria-Black, Arboria-Medium, e opcionalmente Arboria-BlackItalic, citada na especificação original mas sem uso identificado ainda no template atual).
