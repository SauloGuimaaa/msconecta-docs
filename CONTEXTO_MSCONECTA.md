...
### Padrão recorrente de falha silenciosa em operações assíncronas/bloqueantes (risco sistêmico, registrado em 2026-08-13, 5º caso em 2026-08-16)
Cinco bugs distintos, todos no mesmo padrão, corrigidos em menos de duas semanas:
...
Nota de ferramenta (achado incidental, não corrigido): o `/status` do `story_listener_pc` responde `{"ok":true,"status":"..."}`, **sem** o campo `uptime` que `health_check_pipeline.py:checar_story_listener_pc()` exige para classificar como `"ok"` — na prática, o serviço saudável às vezes é classificado como `"falha"` por assinatura incompleta, não por estar realmente fora do ar (confirmado em 2026-08-16: `/status` "falha" ao mesmo tempo em que `/fetch-url` respondia perfeitamente). Vale revisar esse critério numa sessão futura.

**Recomendação — item (a) implementado em 2026-08-25, item (b) segue pendente de decisão do Saulo**: até 2026-08-25 não existia nenhum alerta automático quando o PC Windows fica inacessível — a detecção dependia de alguém tentar usar uma funcionalidade dependente dele e notar a falha (ou rodar `health_check_pipeline.py` manualmente). (a) **Feito**: o cron de `health_check_pipeline.py` foi aprovado/ativado a cada 5min como planejado na Fase 1 (ver seção 3.9), com o histórico agora sendo gravado continuamente em `system_health`/visível no dashboard. (b) **Ainda não implementado**: um alerta via Telegram quando `story_listener_pc`/`render_server_pc` passarem de `ok` para `desconhecido`/`falha` por N checagens seguidas (mesmo padrão de "alerta só em mudança de estado" que `monitor_saude.py` já implementa para os serviços da VPS) — sem isso, a detecção continua existindo só no dashboard (passiva), não empurrada pra Saulo; sem esse alerta, uma queda futura de qualquer outro serviço do PC (ou uma recorrência de `render_server_pc` por uma causa diferente da já corrigida abaixo) ainda dependeria de detecção manual.

**Nota (2026-08-25):** a causa raiz específica por trás da queda recorrente de `render_server_pc`/porta 3001 que motivou boa parte da investigação deste dia foi identificada e corrigida no PC Windows no mesmo dia — ver subseção "Padrão recorrente: watchdog 'vivo' mas estruturalmente incapaz de agir" logo abaixo. A frequência dessa queda específica deve cair para praticamente zero daqui em diante; o item (b) acima segue relevante para qualquer falha futura não coberta por esse fix pontual.
