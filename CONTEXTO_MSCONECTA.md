...
### Padrão recorrente de falha silenciosa em operações assíncronas/bloqueantes (risco sistêmico, registrado em 2026-08-13, 4º caso em 2026-08-16)
Quatro bugs distintos, todos no mesmo padrão, corrigidos em menos de duas semanas:
...
- **2026-08-16** — PC Windows ficou **offline no Tailscale por pelo menos 19h seguidas** (confirmado via `tailscale status`, "last seen" sem avançar mesmo após novas tentativas de conexão da VPS) sem nenhum alerta chegar a Saulo. Efeito em cascata: (a) o fluxo manual de link colado no Telegram estourava o timeout de `gerar_tudo.py` (ver bug de código acima); (b) `monitor_noticias.py` (cron a cada 30min) ficou **incapaz de sequer ler a listagem do site** desde as 09:00 (PC timeout + os 2 proxies públicos de fallback retornando erro 522/520 do Cloudflare) — ou seja, o sistema ficou "cego" para notícias novas por horas, sem nenhuma notificação disso; Saulo só soube da queda ao tentar usar o bot manualmente. Adicionalmente, ao tentar validar a recuperação do PC, descoberto que `health_check_pipeline.py` (criado exatamente para detectar esse tipo de situação, Fase 1 do pipeline, 2026-08-14) tinha um bug de schema (`CHECK` do banco não incluía o status `'desconhecido'` que o próprio script gera para timeout de rede) que o fazia **falhar por completo e não gravar nada** sempre que o PC estivesse instável — o pior momento possível para essa ferramenta falhar (corrigido nesta mesma sessão, ver `HISTORICO_MUDANCAS.md`).

**Recomendação (não implementada ainda, precisa de decisão de Saulo)**: hoje não existe nenhum alerta automático quando o PC Windows fica inacessível — a detecção depende de alguém tentar usar uma funcionalidade dependente dele e notar a falha (ou, a partir de 2026-08-16, rodar `health_check_pipeline.py` manualmente). Como o cron de `health_check_pipeline.py` ainda não foi aprovado/ativado (ver seção 3.9), considerar: (a) aprovar esse cron para rodar a cada 5min como planejado na Fase 1, e (b) adicionar um alerta via Telegram quando `story_listener_pc`/`render_server_pc` passarem de `ok` para `desconhecido`/`falha` por N checagens seguidas (mesmo padrão de "alerta só em mudança de estado" que `monitor_saude.py` já implementa para os serviços da VPS) — sem isso, o padrão tende a se repetir a cada queda futura do PC.
